...
type Decrypt struct {
	Channel struct {
		...
		Entries []*EntryDecrypt `xml:"item"`
	} `xml:"channel"`
}

func FetchDecrypt(ctx context.Context, s *models.Source) error {
	now := time.Now()
	...
	var decrypt Decrypt
	err = xml.NewDecoder(resp.Body).Decode(&decrypt)
	if err != nil {
		return fmt.Errorf("xml decode error: %w", err)
	}

	feed := decrypt.Channel
